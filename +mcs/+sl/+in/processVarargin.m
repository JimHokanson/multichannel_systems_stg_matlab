...
if populate_extras_flag
    extras = mcs.sl.in.process_varargin_result(in,v);
else
    extras = [];
end
...
elseif iscell(v) && length(v) == 1 && isempty(v{1})
    ...
    parse_input = true;
    v = mcs.sl.in.propValuePairsToStruct(v,'allow_spaces',c.allow_spaces);
end
...
fn__new_values   = fieldnames(v); %fn => fieldnames
...
if ~all(is_present)
    if c.allow_non_matches
        ...
        fprintf(2,['\nBad variable names given in input structure: ' ...
            '\n-----------------------------------\n  %s' ...
            '\n\n'],...
            mcs.sl.cellstr.join(badVariables,'d','\n  '))
        fprintf(2,['Available variable names: ' ...
            '\n----------------------------------- \n  %s' ...
            '\n----------------------------------- \n\n'],...
            mcs.sl.cellstr.join(fn__input_struct,'d','\n  '))
        
        error('mcs:sl:in:processVarargin:bad_variables','Bad input variables, see above for options')
    end
end
